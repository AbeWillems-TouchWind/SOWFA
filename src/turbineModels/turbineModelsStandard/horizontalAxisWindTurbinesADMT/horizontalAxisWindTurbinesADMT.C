...
horizontalAxisWindTurbinesADMT::horizontalAxisWindTurbinesADMT
(
...
{   
    ...
    for(int i = 0; i < numTurbinesDistinct; i++)
    {
        ...
        else if (TeeterControllerType[i] == "flapequation")
        {
            ...
            flapODEs.append(flapODE(TeeterI[i] / fluidDensity[i], TeeterCa[i] / fluidDensity[i], TeeterKa[i] / fluidDensity[i], TeeterKcf[i] / fluidDensity[i]));
    ...
    for(int i = 0; i < numTurbines; i++)
    {
        ...
        bladePlaneTip.append(rotorApex[i]);
        bladePlaneTip[i].x() = bladePlaneTip[i].x() + (OverHang[j] + UndSling[j]) * Foam::cos(ShftTilt[j]) + TipRad[j] * Foam::sin(ShftTilt[j]);
        bladePlaneTip[i].z() = bladePlaneTip[i].z() + (OverHang[j] + UndSling[j]) * Foam::sin(ShftTilt[j]) + TipRad[j] * Foam::cos(ShftTilt[j]);
    }
    ...
    for(int i = 0; i < numTurbines; i++)
    {
        ...
        // Define the vector aligned with the blade plane pointing from
        // the rotor apex upwards. 
        uvBladePlane.append(bladePlaneTip[i] - rotorApex[i]);
        uvBladePlane[i] = uvBladePlane[i]/mag(uvBladePlane[i]);
        ...
        pitchSecAvg.append(List<List<scalar> >(nAvgSector[i]));
        for(int m = 0; m < nAvgSector[i]; m++)
        {
            ...
            pitchSecAvg[i][m].append(List<scalar>(nRadial[i],0.0));
        }
...
void horizontalAxisWindTurbinesADMT::rotateBlades()
{  
...
void horizontalAxisWindTurbinesADMT::yawNacelle()
{
    // Perform rotation turbine by turbine.
    forAll(uvTower, i)
    {
	// Rotate the noteworthy points first
    rotorApex[i] = rotatePoint(rotorApex[i], towerShaftIntersect[i], uvTower[i], deltaNacYaw[i]);
    bladePlaneTip[i] = rotatePoint(bladePlaneTip[i], towerShaftIntersect[i], uvTower[i], deltaNacYaw[i]);

	// Recompute the shaft unit vector since the shaft has rotated.
	uvShaft[i] = rotorApex[i] - towerShaftIntersect[i];
	uvShaft[i] = (uvShaft[i]/mag(uvShaft[i])) * uvShaftDir[i];

    // Recompute the blade plane unit vector pointing from
    // the rotor apex upwards. 
    uvBladePlane[i] = bladePlaneTip[i] - rotorApex[i];
    uvBladePlane[i] = uvBladePlane[i]/mag(uvBladePlane[i]);
...
void horizontalAxisWindTurbinesADMT::computeRotSpeed()
{
...
void horizontalAxisWindTurbinesADMT::filterRotSpeed()
{
    // Proceed turbine by turbine.
    forAll(rotSpeedF, i)
    {
...
void horizontalAxisWindTurbinesADMT::controlGenTorque()
{
    // Proceed turbine by turbine.
    forAll(torqueGen, i)
    {
...
void horizontalAxisWindTurbinesADMT::controlNacYaw()
{
    // Proceed turbine by turbine.
...
void horizontalAxisWindTurbinesADMT::controlBladePitch()
{
    // Proceed turbine by turbine.
    forAll(pitch, i)
    {
...
void horizontalAxisWindTurbinesADMT::findControlProcNo()
{
...
void horizontalAxisWindTurbinesADMT::computeWindVectors()
{
...
void horizontalAxisWindTurbinesADMT::computeBladeForce()
{
    ...
    forAll(windVectors, i)
    {
        ...
        // Set the total thrust and moment of the turbine to zero.  Thrust will be summed on a blade-element-
        // wise basis.
        thrust[i] = 0.0;
        moment[i] = 0.0;
        ...
        // For a two bladed system, calculate opposite blade position, this will be used in the moment 
        // calculation. 
...
void horizontalAxisWindTurbinesADMT::computeBodyForce()
{  
...
void horizontalAxisWindTurbinesADMT::computeSectorAverage()
{
            ...
            forAll(alphaSecAvg[i][j], k)
            {
                ...
                forAll(sectorIndices[i][k], m)
                {
                    if (sectorIndices[i][k][m] == j)
                    {
                        ...
                        pitchSecAvg[i][j][k] += pitch[i][k][m] / (scalar(nAzimuth[i][k])/scalar(nAvgSector[i]));
                    }
                }
            }
        }
...
vector horizontalAxisWindTurbinesADMT::XYZtoPolar(vector point, vector centerPoint, vector axis, vector perpaxis)
{
...
vector horizontalAxisWindTurbinesADMT::rotatePoint(vector point, vector rotationPoint, vector axis, scalar angle)
{
...
scalar horizontalAxisWindTurbinesADMT::interpolate(scalar xNew, DynamicList<scalar>& xOld, DynamicList<scalar>& yOld)
{
    label index = 0;
    label indexP = 0;
    label indexM = 0;
...
label horizontalAxisWindTurbinesADMT::interpolate(scalar xNew, DynamicList<scalar>& xOld, DynamicList<label>& yOld)
{
    label index = 0;
    label indexP = 0;
    label indexM = 0;
...
scalar horizontalAxisWindTurbinesADMT::compassToStandard(scalar dir)
{
    dir += 180.0;
    if (dir >= 360.0)
    {
...
scalar horizontalAxisWindTurbinesADMT::standardToCompass(scalar dir)
{
    dir = 90.0 - dir;
    if (dir < 0.0)
    {
...
void horizontalAxisWindTurbinesADMT::openOutputFiles()
{
    if (Pstream::master())
    {
        ...
        *pitchFile_ << "#Turbine    Time(s)    dt(s)    blade pitch angle (degrees)" << endl;
        ...
        // Create a teeter angle file.
        teeterFile_ = new OFstream(rootDir/time/"teeter");
        *teeterFile_ << "#Turbine    Time(s)    dt(s)    teeter angle (degrees)" << endl;
...
void horizontalAxisWindTurbinesADMT::printOutputFiles()
{
    if (Pstream::master())
    {
        forAll(alphaSecAvg,i)
...
volVectorField& horizontalAxisWindTurbinesADMT::force()
{
