...
#include "horizontalAxisWindTurbinesADMUniform.H"
...
horizontalAxisWindTurbinesADMUniform::horizontalAxisWindTurbinesADMUniform
(
...
void horizontalAxisWindTurbinesADMUniform::computeWindVectors()
{
...
void horizontalAxisWindTurbinesADMUniform::computeBladeForce()
{
    ...
    forAll(windVectors, i)
    {
        ...
        // Compute force created with simplified calc.
        TotalForceTurbineSimplified[i] = -0.5 * Ct[i] * Foam::pow(VmagNAvg[i],2) * Foam::constant::mathematical::pi * Foam::pow(TipRad[m],2);
        ...
        // Compute power generated simplified.
        powerRotorSimplified[i] = 0.5 * XChi[i] * Ct[i] * Foam::pow(VmagNAvg[i],3) * Foam::constant::mathematical::pi * Foam::pow(TipRad[m],2);

        // Put the simplified force back into the grid uniformly distributed.        
        // Proceed blade by blade.
        forAll(windVectors[i], j)
        {
            ...
            forAll(windVectors[i][j], k)
            {
                // Distribute this force uniformly along all blade force locations
                bladeForceSimplified[i][j][k] = (TotalForceTurbineSimplified[i] / totDiskPoints[i]) * bladeAlignedVectors[i][j][k][0];


                // Find the component of the blade element force/density in the axial (along the shaft)
                // direction.
                axialForceSimplified[i][j][k] = -bladeForceSimplified[i][j][k] & uvShaft[i];
...
void horizontalAxisWindTurbinesADMUniform::computeBodyForce()
{  
    ...
    forAll(bladeForceSimplified, i)
    {
        ...
        {
            // For each blade.
            forAll(bladeForceSimplified[i], j)
            {
                // For each blade point.
                forAll(bladeForceSimplified[i][j], k)
                {
        ...
        thrustSum += thrustSimplified[i];
        torqueSum += torqueRotorSimplified[i];
    }
            ...
            forAll(windVectorsSecAvg[i][j], k)
            {
                ...
                forAll(sectorIndices[i][k], m)
                {
                    if (sectorIndices[i][k][m] == j)
                    {
                        ...
                        VmagNSecAvg[i][j][k] += VmagN[i][k][m] / (scalar(nAzimuth[i][k])/scalar(nAvgSector[i]));
...
scalar horizontalAxisWindTurbinesADMUniform::interpolate(scalar xNew, DynamicList<scalar>& xOld, DynamicList<scalar>& yOld)
{
    label index = 0;
    label indexP = 0;
    label indexM = 0;
...
label horizontalAxisWindTurbinesADMUniform::interpolate(scalar xNew, DynamicList<scalar>& xOld, DynamicList<label>& yOld)
{
    label index = 0;
    label indexP = 0;
    label indexM = 0;
...
scalar horizontalAxisWindTurbinesADMUniform::compassToStandard(scalar dir)
{
    dir += 180.0;
    if (dir >= 360.0)
    {
...
scalar horizontalAxisWindTurbinesADMUniform::standardToCompass(scalar dir)
{
    dir = 90.0 - dir;
    if (dir < 0.0)
    {
...
void horizontalAxisWindTurbinesADMUniform::update()
{
...
void horizontalAxisWindTurbinesADMUniform::openOutputFiles()
{
    if (Pstream::master())
    {
        ...
        // Create total thrust file 
        TotalForceTurbineSimplifiedFile_ = new OFstream(rootDir/time/"totalthrustsimple");
        *TotalForceTurbineSimplifiedFile_ << "#Turbine    Time(s)    dt(s)    thrust (N)" << endl;
        
        // Create a simplified total thrust file.
        thrustSimplifiedFile_ = new OFstream(rootDir/time/"thrustsimple");
        *thrustSimplifiedFile_ << "#Turbine    Time(s)    dt(s)    thrust (N)" << endl;
        ...
        // Create simplified power file. 
        powerSimplifiedFile_ = new OFstream(rootDir/time/"powerGeneratorSimplified");
        *powerSimplifiedFile_ << "#Turbine    Time(s)    dt(s)    rotor generator (W)" << endl;
        ...
        // Create VmagN file
        VmagNFile_ = new OFstream(rootDir/time/"VmagN");
        *VmagNFile_ << "#Turbine    Sector    Time(s)    dt(s)    Normal Velocity component (m/s)" << endl;
...
void horizontalAxisWindTurbinesADMUniform::printOutputFiles()
{
    if (Pstream::master())
    {
        forAll(windVectorsSecAvg,i)
        {
            ...
            *TotalForceTurbineSimplifiedFile_ << TotalForceTurbineSimplified[i]*fluidDensity[i] << endl;
            // *powerRotorFile_ << powerRotor[i]*fluidDensity[i] << endl;
            // *powerGeneratorFile_ << powerGenerator[i] << endl;
            *powerSimplifiedFile_ << powerRotorSimplified[i]*fluidDensity[i] << endl;
        ...
        // *torqueGenFile_ << endl;
        // *thrustFile_ << endl;
        *TotalForceTurbineSimplifiedFile_ << endl;
        // *powerRotorFile_ << endl;
        // *powerGeneratorFile_ << endl;
        *powerSimplifiedFile_ << endl;
        // *rotSpeedFile_ << endl;
        // *rotSpeedFFile_ << endl;
        // *azimuthFile_ << endl;
        // *pitchFile_ << endl;
...
void horizontalAxisWindTurbinesADMUniform::printDebug()
{
...
volVectorField& horizontalAxisWindTurbinesADMUniform::force()
{
